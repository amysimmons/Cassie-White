...
function themeum_project_slider_shortcode($atts, $content)
{
	...
	foreach ($myprojects as $post){
		...
		if (has_post_thumbnail($post->ID)) {
			...
			$image_info 	= wp_get_attachment( get_post_thumbnail_id($post->ID) );
			$image_desc		= $image_info['description'];

			if( !empty($image_desc) ) {
				$output .= '<a href="'.$image_desc.'" title="'.$post->post_title.'"><img class="img-responsive" src="'.$thumb[0].'" title="'.$post->post_title.'" alt="'.$post->post_title.'"></a>';
			}
			else {
				$output .= '<img class="img-responsive" src="'.$thumb[0].'" title="'.$post->post_title.'" alt="'.$post->post_title.'">';
			}	
